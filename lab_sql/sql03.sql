...
select deptno, ename, sal from emp where deptno in (10, 20) order by deptno; --> deptno에 10 또는 20의 번호가 있으면
...
--> SQL에서 문자열을 작은따옴표('')를 사용. 대소문자를 구분.

-- 직원 테이블에서 업무가 'CLERK' 또는 'MANAGER'인 직원들의
-- 업무, 이름, 급여를 검색. 정렬 순서: (1) 업무, (2) 금여.
...
select empno, ename, job, sal from emp where job != 'CLERK' and job != 'ANALYST' and job != 'MANAGER' order by empno;
select empno, ename, job, sal from emp where job not in ('CLERK', 'ANALYST', 'MANAGER') order by empno; --> job에 CLERK, ANALYST, MANAGER가 없으면
...
select * from emp where job like '_LERK'; --> _ERK는 검색안됨(언더스코어 부족)

-- 이름에 'A'가 포함된 직원들의 모든 정보:
select * from emp where ename like '%A%'; --> '%a%'는 검색안됨(대소문자를 구분)
