...
//		ArrayList<int> a; // 기본타입을 사용했으므로 오류 발생
		...
		// ArrayList에 정수들을 저장. (add를 하는 순서에 따라 index가 결정된다)
		numbers.add(100);
		numbers.add(1);
		numbers.add(-100);
		System.out.println("size = " + numbers.size()); // list는 배열의 length말고 size를 사용
		...
		System.out.println(numbers); // list는 toString()으로 override 되어 있기 때문에 배열과 달리 주솟값이 아닌 저장되어 있는 문자열 그대로 출력
		...
		// ArrayList의 원소 수정
		numbers.set(0, 7); // 앞에는 인덱스, 뒤쪽은 수정할 값.
		System.out.println(numbers);
