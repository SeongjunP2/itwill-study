...
public class LamdaMain03 {
	...
	public static void main(String[] args) {
		...
		// stream은 데이터가 지나가는 통로, filter는 리턴할 값, toList는 앞서 나온 값들을 리스트로 반환 
		...
		// languages.의 원소들 중 길이 5글자인 문자열을 대문자로 변환한 리스트
		List<String> fiveUpper = languages.stream().filter((x) -> x.length() >= 5).map((x) -> x.toUpperCase()).toList();
